...
(function () {
	...
	(function () {
		...
		for (i = 0; i < thisIsMeObj.length; i += 1) {
			//make the html
			$('<div />', {html: "&nbsp;&nbsp;<b>" + thisIsMeObj[i].name + "</b>, " + thisIsMeObj[i].year + ' <a href=# id=slides_' + i + '> This is Me</a><div class="slide" style=' + "width:" + 702 / scale + "px;height:" + 540 / scale + 'px;overflow-y: hidden;overflow-x: hidden;margin-left: auto;boarder:5px solid black;margin-right: auto" id="slideContent' + i + '"></div>'}).appendTo(thisIsMeDiv);
			...
			for (j = 0; j < thisIsMeObj[i].images.length; j += 1) {
				$('<img />', {style: "position:relative;width:" + 702 / scale + "px;", src: 'https://raw.github.com/adussaq/ASPA2/master/thisIsMeImages/' + thisIsMeObj[i].images[j]}).click(function (evt) {
					$(this).hide('slow');
