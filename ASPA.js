...
(function () {
	...
	(function () {
		...
		for (i = 0; i < thisIsMeObj.length; i += 1) {
			//make the html
			$('<div />', {html: "&nbsp;&nbsp;<b>" + thisIsMeObj[i].name + "</b>, " + thisIsMeObj[i].year + ' <a href=# id=slides_' + i + '> This is Me</a><div class="slide" style=' + "height:" + 702 / scale + "px;width:" + "height:" + 540 / scale + 'overflow-y: hidden;overflow-x: hidden;display: block;margin-left: auto;margin-right: auto" id="slideContent' + i + '"></div>'}).appendTo(thisIsMeDiv);
			...
			for (j = 0; j < thisIsMeObj[i].images.length; j += 1) {
				$('<img />', {style: "height:" + 702 / scale + "px;width:" + "height:" + 540 / scale + "px;display: block;margin-left: auto;margin-right: auto", src: 'https://raw.github.com/adussaq/ASPA2/master/thisIsMeImages/' + thisIsMeObj[i].images[j]}).click(function (evt) {
					$(this).hide('slow');
